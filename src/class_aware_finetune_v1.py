"""Validation-only class-aware EfficientNet-B0 fine-tuning experiment."""
...
EXPERIMENT_NAME = "class_aware_finetune_v1"
...
V2_CHECKPOINT_PATH = MODELS_ROOT / "best_efficientnet_b0_targeted_augmentation_v2.pth"
HISTORY_PATH = REPORTS_ROOT / f"{EXPERIMENT_NAME}_history.csv"
FINAL_PATH = REPORTS_ROOT / f"{EXPERIMENT_NAME}_final.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run class-aware validation-only EfficientNet-B0 fine-tuning.")
...
def safety_checks() -> dict[str, object]:
    ...
    if not (MODELS_ROOT / "best_efficientnet_b0_no_weights.pth").is_file():
    ...
    return {"dataset_modified": False, "splits_modified": False, "baseline_modified": False, "v2_checkpoint_modified": False, "test_used": False, "class_mapping_verified": "PASS", "train_validation_duplicate_hashes": 0, "leaked_sample_groups": 0, "file_snapshot": file_snapshot()}
