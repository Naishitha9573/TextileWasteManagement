...
async def analyze_batch_image(
    ...
    # Store classification report alongside analysis response for API consumers
    analysis_result_payload = {
        "classification_report": classification_report,
        "recycling_strategy": recs["strategy"],
        "recycling_options": recs["options"],
        "sustainability_summary": sustainability_result,
    }
    ...
    response_payload = {
        ...
        "analysis": {
            ...
            "material_recovery_score": analysis.material_recovery_score,
            "overall_circularity_score": analysis.overall_circularity_score,
            "circularity_category": analysis.circularity_category,
...
@app.post("/api/predict/material")
def predict_material(payload: schemas.MaterialPredictionRequest, current_user: User = Depends(get_current_user)):
    batch = SimpleNamespace(
        id=0,
        fabric_type=payload.fabric_type,
        condition=payload.condition,
        quantity=payload.quantity,
        source=payload.source or "Manual prediction",
    ...
    classification_report = algorithms.generate_classification_report(
        batch=batch,
        image_bytes=None,
        filename=f"{payload.fabric_type}.png",
        cv_features=cv_features,
    )
    sustainability_result = sustainability_service.analyze_material(
    ...
    return {
        ...
        "classification_report": classification_report,
        "recommendation": sustainability_result["recommendation"],
        "scores": sustainability_result["scores"],
        "environmental_impact": sustainability_result["environmental_impact"],
        "confidence": classification_report["material_classification"]["confidence"],
    }
...
# --- CLASSIFICATION REPORT ENDPOINTS ---
@app.get("/api/reports/waste-classification/{id}")
def waste_classification_report(id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    batch = db.query(WasteBatch).filter(WasteBatch.id == id).first()
    if not batch:
        raise HTTPException(status_code=404, detail="Batch not found")
    analysis = db.query(AnalysisResult).filter(AnalysisResult.batch_id == batch.id).first()
    ...
    return {
        ...
        "fabric_type": batch.fabric_type,
        "status": batch.status,
        "material_classification": classification_report["material_classification"],
        "waste_category": classification_report["waste_category"],
        "recyclability_assessment": classification_report["recyclability_assessment"],
        "processing_notes": classification_report["processing_notes"],
    }
