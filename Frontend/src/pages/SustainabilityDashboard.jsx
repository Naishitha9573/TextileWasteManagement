...
export default function SustainabilityDashboard() {
  ...
  const downloadESGReport = () => {
    ...
    doc.setFontSize(22);
    doc.text("Corporate Sustainability & ESG Action Report", 14, 25);
    
    doc.setFontSize(10);
    doc.setFont("Inter", "normal");
    doc.text(`Report Period: FY2026 Q3 | Generated: ${new Date().toLocaleDateString()}`, 14, 32);
    ...
    doc.setFont("Outfit", "bold");
    doc.setFontSize(14);
    doc.text("2. Key ESG Metrics & Impact Mitigation", 14, 76);
    
    const impactData = [
      ["Metric", "Value achieved", "Environmental Significance"],
      ["Carbon Footprint Mitigation", `${analytics.co2_saved_kg.toLocaleString()} kg CO2`, "Equivalent greenhouse gases offset from raw manufacturing"],
      ["Hydric Resource Preservation", `${analytics.water_saved_liters.toLocaleString()} Liters`, "Water offset from pesticide/growth processes in cotton/linen"],
      ["Landfill Diversion Weight", `${analytics.landfill_diverted_kg.toLocaleString()} kg`, "Solid textile waste completely prevented from entering standard landfills"],
      ["Waste Diversion Efficiency", `${analytics.diversion_rate}%`, "Percentage of registered textile batches successfully recycled/reused"],
      ["Average Circularity Index", `${analytics.circularity_avg}%`, "Average recovery score across all processed textile components"]
    ];
    doc.autoTable({
      head: [impactData[0]],
    ...
    doc.setFont("Outfit", "bold");
    doc.setFontSize(14);
    doc.text("3. Sustainability Benchmark Achievements", 14, doc.lastAutoTable.finalY + 15);
